<script lang="ts">
	...
	onMount(async () => {
		try {
			// Load channel info
			channel = await api.getChannel(params.channelId);
			
			// Load messages
			const msgs = await api.getMessages(params.channelId);
			messages = msgs;
	...
	async function handleSendMessage(e: Event) {
		...
		if ($wsClient.connected) {
			...
			// Fallback to HTTP
			try {
				const msg = await api.createMessage(params.channelId, content);
...
<main class="h-screen flex flex-col bg-base text-text">
	...
	<div class="border-b border-border bg-surface">
		<div class="px-4 py-4 flex items-center gap-4">
			<a href="/servers/{params.id}" class="text-sm text-subtext hover:text-text">← Back</a>
			{#if channel}
			...
			{:else}
				{#each messages as message}
					<div class="flex gap-3">
						<div class="flex-shrink-0 w-10 h-10 rounded-full bg-blue flex items-center justify-center text-sm font-medium">
							{message.author_id.slice(0, 2).toUpperCase()}
						</div>
						<div class="flex-1 min-w-0">
							<div class="flex items-baseline gap-2 mb-1">
								<span class="text-sm font-medium">{message.author_id.slice(0, 8)}</span>
								<span class="text-xs text-subtext">{formatTime(message.created_at)}</span>
